...
void LC_vec(real * res, real * const x1, real * const x2, real const s1, real const s2, int const n)
{
  ...
  if (n>40){
      cblas_dscal(n, s1, x1, 1);
      cblas_dscal(n, s2, x2, 1);
      vdAdd(n, x1, x2, res);
  }
  else{
      for (int ii = 0; ii < n; ++ii)
...
void PacLPsolver(real ** Y, real const epsi, real const * const a, int const m, int const n)
{
    ...
    printf("Reached iteraton %d. Elapsed time %f\n", T, now-start);
    fflush(stdout);
...
{
	int n = 40;			/* Dimensionality */
	int m = 60;			/* Number of constraints */
	...
	while ((e = getopt(argc, argv, optstring)) != -1) {
	  switch (e) {
	  case 'h':
            fprintf(stderr, "Something is messed up here.");
            return -1;
